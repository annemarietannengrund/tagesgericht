...
class TestCalendarweek(TestCase):
    ...
    def test_get_cw_from_to_called(self, get_cw_from_to):
        Calendarweek(year="2021", week="42")
    ...
    def test_get_items(self):
        cw = Calendarweek(year="2021", week="42")
        cw.items = {'testitem': 'unittest'}
        items = cw.get_items()
        self.assertEqual({'testitem': 'unittest'}, items)
    ...
    def test_add_file(self, load_message, initialize):
        cw = Calendarweek(year="2021", week="42")
        cw.add_file(filepath='unittest', day_num=0, item_date=date(2021, 6, 13))
...
class TestCalendarweekManager(TestCase):
    ...
    def test_get_today_from_calendarweek(self, init_manager, lread_file):
        cwm = TagesgerichtManager(
            weekday_map=self.weekday_map,
            active_days=self.active_days,
            data_dir=self.data_dir,
